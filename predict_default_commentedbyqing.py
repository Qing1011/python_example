...
if __name__ == "__main__":
    ...
    for race in [0]:
        ...
        df2_linear = df[[outcome] + explanatory_vars_linear].dropna()
        df2_nonlinear = df[[outcome] + explanatory_vars_nonlinear].dropna()
        df2_linear = df2_linear.apply(pd.to_numeric)
        df2_nonlinear = df2_nonlinear.apply(pd.to_numeric)
        ...
        y_linear = df2_linear[outcome]
        y_nonlinear = df2_nonlinear[outcome]
        if race == 1:
        ...
        predicted_vals_all = pd.DataFrame()
        if int_rate == 0:
        ...
        if cv:
            ...
            grid_clf = GridSearchCV(clf, param_grid, cv=3, verbose=1, n_jobs = 40)
            grid_clf.fit(X_train, y_train)
            clf_tree = grid_clf.best_estimator_
            ...
            print "Forest CV"
            grid_clf.fit(X_train, y_train)
            clf_forest = grid_clf.best_estimator_
            param_grid = {
                'n_estimators' : [40,80],
        ...
        for name, clf in zip(names, classifiers):
            if name == "LogitNonLinear":
                ...
                result = pd.DataFrame(data=clf.predict_proba(X_test2))
                result_all = pd.DataFrame(data=clf.predict_proba(df3_nonlinear))
                fpr[name], tpr[name], _ = roc_curve(y_test, result[1])
                fpr[name], tpr[name], _ = precision_recall_curve(y_test, result[1])
                ...
                fpr[name], tpr[name], _ = roc_curve(test_vals.iloc[:,0], results_roc[1])
            ...
            predicted_vals_all = pd.concat([predicted_vals_all.reset_index(drop=True), result_all[1].reset_index(drop=True)], axis=1, ignore_index=True)
